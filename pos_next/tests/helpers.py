"""Shared test helpers and site-independent fixtures for pos_next.

Ported from selling_additional's tests/helpers.py. The ownership constants are
imported from :mod:`pos_next.ownership` — the single source of truth — rather
than duplicated here. (The source app defined them locally only because its
``ownership`` module did not exist yet; pos_next owns these fields natively, so
re-declaring them here would just risk drift and is what task 1.3 flags to fix.)
"""
...
def get_default_company() -> str:
	...
	doc = frappe.get_doc(
		{
			"doctype": "Company",
			"company_name": "_Test Selling Company",
...
def item_group() -> str:
	...
	if not parent:
		root = frappe.get_doc(
			{"doctype": "Item Group", "item_group_name": "_Test SA Root Group", "is_group": 1}
		)
	...
	group = frappe.get_doc(
		{
			"doctype": "Item Group",
			"item_group_name": "_Test SA Leaf Group",
			"is_group": 0,
