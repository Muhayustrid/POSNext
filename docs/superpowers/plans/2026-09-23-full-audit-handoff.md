...
| 3 | COR-FE-01, COR-BE-02..05, SEC-NEW-01..03 | **SELESAI & terverifikasi** (22 Sep) | BELUM commit. COR-FE-01: guard `submitInFlight` + `isSubmitting` jalur offline + backstop dialog (vitest 478/478). Gates: owner-or-read `get_pos_invoices`, `_assert_profile_access` di `create_production`, filter+gate draft (`get_draft_invoices`, lane manajer = role-set karena kasir pegang read doc-wide). Uang: ekstraksi `_validate_item_rates` dipanggil di branch draft-submit (+tutup SEC-04 tanpa harga server), reversal wallet per-baris fail-closed, redeem credit re-raise (wajib post-submit per `credit_sales.py:272`; rollback txn menutup), pointer offline-sync idempoten + bersih saat reuse. Review: 2 MAJOR diterapkan (basis harga wajib `pos_profile_doc` — vektor `selling_price_list` palsu; `_reuse_sync_record` hapus pointer mati — duplikat pasca-cancel) + MINOR `credit_return_to_wallet` fail-closed + WT gagal masuk pesan throw; semua dengan bukti RED→GREEN. Catatan: gate jadwal `assert_invoice_sales_allowed` kini dievaluasi 2× di jalur credit (fail-closed, disengaja). **Utang follow-up (pre-existing)**: `POSNext Cashier` tak punya `write` di POS Production Log → alur `create_production` kasir gagal di `log.submit()` sejak sebelum audit; `reverse_wallet_transactions_for_return` masih hardcode "Sales Invoice". |
| 4 | COR-FE-02..05 (frontend correctness) | **SELESAI & terverifikasi** (22 Sep) | Commit "audit grup 4". COR-FE-02 draft round-trip diskon+kupon (+clamp max_amount hasil review); COR-FE-03 clearCart boundary shift (+fix ReferenceError `_initializedProfile` laten); COR-FE-04 sync_failed exclude auto-sync + backoff + Retry/Hapus di dialog (worker=display only, berkomentar); COR-FE-05 SW `/sw.js` controller `www/sw.py` scope `/` — **GUI-verified**: scope `/`, activated, controller mengendalikan `/pos`, precache 62 absolut, sweep unregister SW lama. Review APPROVE 0 MAJOR; vitest 496/496. Utang follow-up opsional (reviewer MINOR #2/#4): guard reload multi-tab saat update SW + `wb.update()` periodik utk tab yang tidak pernah navigasi. |
| 5 | PERF-01..06 + SEC-NEW-04 | **SELESAI & terverifikasi** (22 Sep) | BELUM commit. PERF-01: cutoff sargable (split date/time; arm IS NULL = superset disengaja — komentar lurus) + window filter ter-push ke branch union; EXPLAIN range posting_date; paritas level handler OLD vs NEW identik. PERF-02: `get_pos_invoices` bulk header+child (payments/taxes) + `get_invoices` bulk item; test paritas per-field vs as_dict. PERF-03: paginasi 500 + delta `modified_since` (cache.js + customerSearch.js + param `start` baru di `customers.py` dengan tiebreaker `name asc`); PERF-04: debounce 250ms + toRaw. PERF-05: price group bulk dict (predikat Python terbukti setara renderer frappe). PERF-06/SEC-NEW-04: room `pos_profile:{name}` via app handler `realtime/handlers.js` + HTTP gate `has_pos_profile_access` (anggota POS Profile User); 4 emitter user=None dihapus; 3 composable SPA via `usePosProfileRoom`; fan-out customer ke profil enabled; **review MAJOR diterapkan**: `stop()` tidak lagi unsubscribe (socket bersama — satu stop dulu bisa mematikan room utk ketiganya). Gate: hq 37 + reports 2 + closing 8 + closing2 + session 37 + backdate 34 + submit 12 + price group 29+3 + realtime 11 + parity 3; vitest 509/509; build OK. Regression disengaja: 2 profil share warehouse → stock update hanya ke profil pemilik. Utang opsional (NIT reviewer): cap halaman sync, predikat `_item_from` bersarang. |
| 6 | Sisa SEC/COR/REL + REL-03/06..10 | **SELESAI & TERVERIFIKASI** (24 Sep) | BELUM commit. SEC-NEW-05..12: gate `get_referral_codes`/credit_sales(2)/wallet(3)+loyalty-throw/`get_create_pos_profile` manager-only/`get_pos_settings` member-or-manager; `get_cashiers` whitelist `parent` + gate; imin_probe.html + route dihapus; `@rate_limit` 20/300s per-IP di `check_code`/`validate_confirmation_code` (RateLimitExceededError 429, key redis dibersihkan test). COR-BE-06: lock opening `for_update` statement pertama + dup-check locking read (index `pos_opening_shift` via `search_index` di JSON, migrate idempoten). COR-BE-07: re-cek one_use per-customer di `increment_coupon_usage` bawah lock. COR-BE-08: mirror `included_in_print_rate` per account_head dari invoice asal. COR-BE-09: tolak return tanpa `return_against` (UI selalu kirim — terverifikasi). COR-BE-10: `_should_block` beda None vs 0. COR-BE-11: `_validate_draft_conflict` modified 409 (+guard timestamp rusak). COR-BE-12: outstanding `for_update` 2 jalur. COR-BE-12b: pricing rule re-raise saat submit. COR-BE-13: `_set_payment_accounts` tanpa blanket except. COR-BE-14: dead branch dihapus. COR-BE-15: `submit_printed_invoices` pindah ke `before_submit` controller + loop API jadi backstop; preview kini `pending_printed_drafts`; **review MAJOR diterapkan**: merge aditif `_merge_late_submitted_invoices` (baris manual Desk dipertahankan, bukan rebuild-total). COR-BE-16: kegagalan konversi loyalty kini throw (rollback request penuh). COR-FE-06: SAR "SR" + desimal per mata uang (IDR tetap 0) DI UI DAN STRUK (`receiptCurrency` + `formatCurrencyNumber` param currency — celah struk ditemukan & ditutup orchestrator saat review). COR-FE-07: timeout tak melepas lock (+review: sync-throw aman via `Promise.resolve().then(fn)`, joiner irisan terbatas). COR-FE-08: ekspor localStorage pra-delete (+review MAJOR: DB tak terbaca → reset tetap jalan; kuota → abort). COR-FE-09: fallback PING false. COR-FE-10: `PostPrintError` chain berhenti + fallback browser mati utk gagal pasca-cetak (+review: driver iMin menandai `err.postPrint` setelah sheet pertama queued). COR-FE-11: `saveOfflinePayment`+`payment_queue` dihapus (Dexie version drop). COR-FE-12: qty 0 divalidasi eksplisit. COR-FE-13: payload "Cash" konstanta. COR-FE-14: `parseDateOnly` lokal. COR-FE-15: timer di-clear onUnmounted. COR-FE-16: `clearBrowserCache` allowlist `pos_next_` saja. SEC-NEW-06: DOMParser. REL-03: uninstall lengkap (CLEANUP_CUSTOM_FIELDS dari install + role/docperm/workspace, coverage test statis; **insiden**: run RED pertama mengeksekusi deleter lama → 10 CF terhapus di dev, dipulihkan penuh — 2 kolom legacy SI kehilangan nilai historis, diverifikasi orchestrator). REL-06: versi **2.12.0** (bukan 2.11.0 sesuai plan — grup 2 sudah bikin patch v2_12_0; test konsistensi `test_release_hygiene`). REL-07: pypdf dihapus (0 import; ignore ruff F401/F403/F405 dievaluasi — dikerek ke CLN). REL-08: 1 baris duplikat stale id.csv dihapus (varian ber-trailing-space tetap — dipakai CouponDialog; "Discount "/"Discount" keduanya sah); full re-sync terjemahan = utang. REL-09: `_asset_version` dead code dihapus. COR-BE-17: `_pn_sync_ws.py` dihapus. Gate: sapuan serial 24 modul = 22 lulus + 2 flake posisi-dependent tak ter-reproduksi (cashier_permissions & uninstall_coverage — lulus individual/berpasangan); **test_promotions terbukti BUKAN regresi grup 6** (10 error identik pada kode 5494e41 via stash; akar = modus `invoice_type` situs + gate R3 pra-ada; eskalasi 2→10 terjadi pra-grup-6); leak mode antar-modul dibereskan (mixin + 5 file restore baseline, bukan hardcode Sales); vitest 552/552; build OK; serve direstart. |
